...
T_sim = 20
T_fig = 2
betahat = 0.75  # new parameter for open economy

# RERD adjustment e_r = e_{r,-1} + (1 + h/theta) * (pi^f - pi)
h = 0.5
theta = 2.0
kappa = 1 + h/theta
...
for t in range(1, T_sim+1):
    ...
    denominator = (1 + betahat*gamma)
    y_vals[t]   = y_bar + (betahat * common) / denominator
    pi_vals[t]  = pi_f_path[t] + s[t] + (gamma * betahat * common) / denominator
    e_r_vals[t] = e_r_vals[t-1] + kappa * (pi_f_path[t] - pi_vals[t])
...
plt.tight_layout()
plt.savefig("as_ad/baseline_impulse_open_economy_flex.pdf")
...
def solve_equilibrium(z_t, s_t, pi_f_t):
    ...
    denominator = (1 + betahat*gamma)
    y           = y_bar + (betahat * common) / denominator
    pi          = pi_f_t + s_t + (gamma * betahat * common) / denominator
    e_r         = e_r_last + kappa * (pi_f_t - pi)
    return y, pi, e_r
...
for i, key in enumerate(ad_keys):
    ...
    ad_vals = e_r_lag + pi_f_vals[rep] - (1/betahat)*(y_grid - y_bar) + z_vals[rep]
...
plt.tight_layout()
plt.savefig("as_ad/baseline_diag_open_economy_flex.pdf")